...
pub struct PracticeRegistry {
    practices: std::collections::HashMap<&'static str, &'static BestPractice>
}

impl PracticeRegistry {
    /// Build registry from all practices.
    pub fn new() -> Self {
        let mut practices = std::collections::HashMap::new();
    ...
    pub fn find(&self, code: &str) -> Option<&'static BestPractice> {
        ...
        self.practices.get(normalized.as_str()).copied()
    }
