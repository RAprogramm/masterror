...
//! ```
//!
//! The generated `From` conversion attaches the domain error as the source of
//! the resulting [`AppError`], so it stays downcastable and visible in
//! `chain()`/`root_cause()`. This requires the domain error to be
//! `Send + Sync + 'static`; add the `no_source` flag to `#[app_error(...)]`
